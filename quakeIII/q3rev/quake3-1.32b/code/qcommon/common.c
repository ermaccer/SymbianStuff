...
static qboolean hunk_created = qfalse;
...
EXTERN_C void HunkMem_Clear(void);
EXTERN_C void HunkMem_SwapBanks(void);
...
EXTERN_C void HunkMem_ClearTemporary(void);
EXTERN_C void HunkMem_SetMark(void);
EXTERN_C void HunkMem_ClearToMark(void);
...
void Com_InitHunkMemory( void ) {
    ...
    err = HunkMem_Create((s_hunkTotal + 31+4095)&~4095);
...
void *Hunk_AllocateTempMemory( int size ) {
	...
	hunk_temp->temp += size;
    buf = HunkMem_AllocTemporary(size);
...
void Hunk_FreeTempMemory( void *buf ) {
	...
	temp = hdr->size;
	if (HunkMem_FreeTemporary(hdr,hdr->size))
	    {
    	hunk_temp->temp -= temp;
	    }
	else
