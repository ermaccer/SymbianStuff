...
void ConvertAndUpload565_16(    unsigned char* pixels,
                                int width, int height, int miplevel )
    {
    unsigned short converted[256*256];
    unsigned short* conversionptr = converted;
    int index = 0;
    for (index = 0; index < width*height; index++)
        {
        *conversionptr++ =  ((pixels[index*4]&0xf8) << 8)   |
                            ((pixels[index*4+1]&0xfc) << 3) |
                            ((pixels[index*4+2]>>3));
        }
    qglTexImage2D (GL_TEXTURE_2D, miplevel, GL_RGB, width, height, 0, GL_RGB, GL_UNSIGNED_SHORT_5_6_5, converted);
    }
                           
void ConvertAndUpload4444_16(    unsigned char* pixels,
                                int width, int height, int miplevel )
    {
    unsigned short converted[256*256];
    unsigned short* conversionptr = converted;
    ...
    for (index = 0; index < width*height; index++)
        {
        *conversionptr++ =  ((pixels[index*4]&0xf0) << 8)   |
                            ((pixels[index*4+1]&0xf0)<< 4)  |
                            ((pixels[index*4+2]&0xf0))      |
                            ((pixels[index*4+3]&0xf0) >>4);
        }
    qglTexImage2D (GL_TEXTURE_2D, miplevel, GL_RGB, width, height, 0, GL_RGBA, GL_UNSIGNED_SHORT_4_4_4_4, converted);
    }

void Upload16Direct(    void* pixels,
                  int width, int height, int miplevel,GLenum internalFormat )
    {
    qglTexImage2D (GL_TEXTURE_2D, miplevel, GL_RGB, width, height, 0, GL_RGB, GL_UNSIGNED_SHORT_5_6_5, pixels);
    }


void Upload16(    void* pixels,
                  int width, int height, int miplevel,GLenum internalFormat )
    {
    if (internalFormat == 3)
        {
        ConvertAndUpload565_16( (unsigned char*)pixels,
                                width, height, miplevel );

        }
    else
        {
        ConvertAndUpload4444_16(   (unsigned char*)pixels,
                                   width, height, miplevel );
        }
    }
						  ...
						  int *pUploadWidth, int *pUploadHeight )
{
	...
	if ( ( scaled_width == width ) && 
		...
		if (!mipmap)
		{
			...
			Upload16( data,scaled_width, scaled_height, 0,internalFormat);
...
//	qglTexImage2D (GL_TEXTURE_2D, 0, internalFormat, scaled_width, scaled_height, 0, GL_RGBA, GL_UNSIGNED_BYTE, scaledBuffer );
    Upload16( scaledBuffer,scaled_width, scaled_height, 0,internalFormat );
	if (mipmap)
	{
		int		miplevel;
...
//			qglTexImage2D (GL_TEXTURE_2D, miplevel, internalFormat, scaled_width, scaled_height, 0, GL_RGBA, GL_UNSIGNED_BYTE, scaledBuffer );
		Upload16( scaledBuffer,scaled_width, scaled_height, miplevel,internalFormat );
		}
	}
done:
