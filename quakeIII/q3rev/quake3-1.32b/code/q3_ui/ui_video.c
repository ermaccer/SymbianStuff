...
static void GraphicsOptions_GetInitialVideo( void )
{
	...
	s_ivo.fsaa = s_graphicsoptions.fsaa.curvalue;
}
...
/*	if ( s_graphicsoptions.allow_extensions.curvalue == 0 ) //[ohi]
	{
	...
	if ( s_ivo.fsaa != s_graphicsoptions.fsaa.curvalue )
	{
		s_graphicsoptions.apply.generic.flags &= ~(QMF_HIDDEN|QMF_INACTIVE);
	}
...
static void GraphicsOptions_ApplyChanges( void *unused, int notification )
{
	...
	switch ( s_graphicsoptions.fsaa.curvalue  )
	{
    default:
	case 0:
		trap_Cvar_SetValue( "r_fsaa", 0 );
		break;
	case 1:
		trap_Cvar_SetValue( "r_fsaa", 1 );
		break;
	}
...
static void GraphicsOptions_Event( void* ptr, int event ) {
	...
	case ID_LIST:
		...
		s_graphicsoptions.fsaa.curvalue        = ivo->fsaa;
...
static void GraphicsOptions_SetMenuItems( void )
{
	...
	switch ( ( int ) trap_Cvar_VariableValue( "r_fsaa" ) )
	{
	default:
	case 0:
		s_graphicsoptions.fsaa.curvalue = 0;
		break;
	case 1:
		s_graphicsoptions.fsaa.curvalue = 1;
		break;
	}
...
void GraphicsOptions_MenuInit( void )
{
	...
	// references/modifies "r_fsaa"
	s_graphicsoptions.fsaa.generic.type = MTYPE_SPINCONTROL;
	s_graphicsoptions.fsaa.generic.name	= "FSAA:";
	s_graphicsoptions.fsaa.generic.flags= QMF_PULSEIFFOCUS|QMF_SMALLFONT;
	s_graphicsoptions.fsaa.generic.x	= 400;
	s_graphicsoptions.fsaa.generic.y	= y;
	s_graphicsoptions.fsaa.itemnames    = enabled_names;
	y += BIGCHAR_HEIGHT+2;
	...
	Menu_AddItem( &s_graphicsoptions.menu, ( void * ) &s_graphicsoptions.fsaa );
