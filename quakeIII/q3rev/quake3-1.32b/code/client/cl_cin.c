...
extern	int		s_rawend;
static int cinematics_playref = 0;
...
static	unsigned short		*vq2 = NULL;//[256*16*4];
static	unsigned short		*vq4 = NULL;//[256*64*4];
static	unsigned short		*vq8 = NULL;//[256*256*4];
...
e_status CIN_StopCinematic(int handle) {
    ...
    if (!cinematics_playref)
        ...
        free(vq4); vq4 = NULL;
        free(vq2); vq2 = NULL;
        free(cin.linbuf); cin.linbuf = NULL;    
        }
...
int CIN_PlayCinematic( const char *arg, int x, int y, int w, int h, int systemBits ) {
    ...
    if (cin.linbuf)
        {
        linbuftemp = cin.linbuf;    
        }
    Com_Memset(&cin, 0, sizeof(cinematics_t) );
    if (linbuftemp)
        {
        cin.linbuf = linbuftemp;    
        }

    if (!cinematics_playref)
        {
        cin.linbuf = (byte*)malloc(DEFAULT_CIN_WIDTH*DEFAULT_CIN_HEIGHT*4*2);
        vq2 = (unsigned short*)malloc(256*16*4*2);
        vq4 = (unsigned short*)malloc(256*64*4*2);
        vq8 = (unsigned short*)malloc(256*256*4*2);    
        }
    cinematics_playref++;
...
void CIN_DrawCinematic (int handle) {
    ...
    buf2 = (unsigned short*)malloc(256*256*2);
    bc2 = buf2;
    bc3 = (unsigned int*)buf;

    if (cinTable[handle].CIN_HEIGHT > 256)
        {
        for (ycount = 0; ycount < 256; ycount++)
            {
            for (xcount = 0; xcount < 256; xcount++)
                {
                colorvalue = *bc3;
                bc3+=cinTable[handle].CIN_WIDTH >> 8;
                *bc2++ = ((colorvalue & 0xf8) << 8) |
                         ((colorvalue & 0xfc00) >> 5) |
                         (colorvalue >> 19);          
                }
                bc3+=cinTable[handle].CIN_WIDTH;
            }            
        }
    else
        {
